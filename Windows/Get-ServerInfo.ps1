...
$OutputLocation3 = "\\site1-CSRPT01\C$\Windows\Web\AppsOnDCs","\\site2-CSRPT01\C$\Windows\Web\AppsOnDCs"

$DataLocation1 = "\\site1-file01\csreports\PatchStatus"
$DataLocation2 = "\\site1-file01\csreports\ServerInfo"
$DataLocation3 = "\\site1-file01\csreports\InstalledApps"
...
Workflow Get-Patchinfo {
    ...
    if ($OSType -match 'Core'){$OS = $CT.Caption+" Core"} else {$OS = $CT.Caption}
    ...
    if ((Get-CimInstance win32_operatingsystem).Caption -match '2016') {$EMETCheck = $AntimalwareCheck = $ScepVer = 'N/A'}
    else
    {
        try {Get-Service EMET_Service -ErrorAction Stop | Out-Null; $EMETCheck=(Get-ChildItem "C:\Program Files (x86)\EMET *" -Recurse | ? {$_.Name -match 'EMET_GUI.exe'}).VersionInfo.ProductVersion} catch {$EMETCheck=$false}
        try {Get-Service MsMpSvc -ErrorAction Stop | Out-Null; $AntimalwareCheck=(Get-ChildItem 'C:\Program Files\Microsoft Security Client\MsMpEng.exe').VersionInfo.ProductVersion} catch {$AntimalwareCheck=$false}
        #try {$ScepVer=(Get-ChildItem 'C:\Program Files\Microsoft Security Client\MsMpEng.exe' -ErrorAction Stop).VersionInfo.ProductVersion} catch {$ScepVer="Not Found"}
    }
    try {Get-Service ccmexec -ErrorAction Stop | Out-Null; $SCCMCheck=$true} catch {$SCCMCheck=$false}
    
    if ($(try {Get-Service CarbonBlack -ErrorAction Stop} catch{})) 
        {$CarbonBlack = $true}
    else {$CarbonBlack = $false}
    if ($(try {Get-Service NxLog -ErrorAction Stop} catch{})) 
        {$NxLog = $true}
    else {$NxLog = $false}
    if ($(try {Get-Service Puppet -ErrorAction Stop} catch{})) 
        {$Puppet = $true}
    else {if (($env:COMPUTERNAME -like "irn*") -or ($env:COMPUTERNAME -like "sv2-*")) {$Puppet = "Exception"} Else {$Puppet = $false} }
    $InstalledApps = Get-CimInstance -ClassName win32_product
    $DuoApp = $InstalledApps | ? {$_.Name -match 'Duo Authentication'}
    if ($DuoApp)
    {
        $Duo = if ($DuoApp.Name -match 'Remote Desktop Gateway') {"Tsg "+$DuoApp.Version} else {$DuoApp.Version}
    }
    else {if ($env:COMPUTERNAME -match '-orl-kofx'){$Duo = "Exception"} Else {$Duo = $false} }
    $SensuApp = $InstalledApps | ? {$_.Name -match 'Sensu'}
    if ($SensuApp) {$Sensu = $SensuApp.Version} 
    else {if (($env:COMPUTERNAME -like "irn*") -or ($env:COMPUTERNAME -like "sv2-*")) {$Sensu = "Exception"} Else {$Sensu = $false} }
    ...
    #Check LAPS
    if ((Get-CimInstance win32_computersystem).Roles -match 'Domain_Controller')
    {
        $LAPS = "N/A"
    }
    Else 
    {
        $LapsVer = (Get-ItemProperty -Path HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | ? {$_.displayname -eq "Local Administrator Password Solution"}).DisplayVersion
        if ($LapsVer) {$LAPS = $LapsVer}
        else {$LAPS = $false}
    }
    if ((Get-Service -Name MpsSvc).Status -eq 'running') 
    {
        $FWStatus = (Get-NetFirewallProfile | % {"$($_.Name)-$($_.Enabled)"}) -join ' '
    }
    else
    {
        $FWStatus = "Svc Stopped"
    }
    $SMB1Enabled = ((Get-SmbServerConfiguration).Enablesmb1protocol -or (Get-WindowsFeature FS-SMB1).Installed)
    New-Object psobject -Property @{
        ...
        } | select PSComputerName,OS,CPUs,Memory,WUServer,AutoUpdate,LastBoot,RebootPending,LastPatch,Updates,UpdatesNeeded,DNSServers,EMET,SCEP,SCCM,CarbonBlack,NxLog,Puppet,Duo,LAPS,Sensu,FWStatus,SMB1Enabled
    }
}
...
write-progress -Activity 'Collecting SErver Information...' -percentComplete ((($Servers.Count - $leftServers)/$Servers.Count)*100) -Status "Completed $($Servers.Count - $leftServers) out of $($Servers.Count) Servers"
} 
Until ((Get-Job -IncludeChildJob | ?{$_.State -eq "running"}).count -eq 0)
$ServerInfo = Get-Job | Receive-Job

#start of local server collection
    $CT = Get-WmiObject win32_operatingsystem
    $OSType = (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion').InstallationType
    if ($OSType -match 'Core'){$OS = $CT.Caption+" Core"} else {$OS = $CT.Caption}
    $LB = Get-Date $ct.ConvertToDateTime((Get-WmiObject win32_operatingsystem).LastBootUpTime) #-Format yyyy/M/dd
    ...
    $ServerInfo += New-Object psobject -Property @{
        PSComputerName = (Resolve-DnsName $env:COMPUTERNAME -Type A).Name
        OS = $OS.replace("Microsoft Windows Server ","").replace("Technical Preview","TP").replace("Datacenter","DC").replace("Standard","Std").replace("Enterprise","Ent")
        LastBoot = $LB
        LastPatch = $LastPatchDate
        Updates = $NumOfUpdates
...
#End of local server collection
...
$Diff | % `
    ...
    if (!(Test-Connection $_ -Count 1 -Quiet))
        {$ServerInfo += New-Object psobject -Property @{PSComputerName = $_; OS = "Not Found"; CPUs = "Not Found"; Memory = "Not Found"; 
        WUServer = "Not Found"; AutoUpdate = "Not Found";LastBoot = "Not Found"; RebootPending = "Not Found"; LastPatch = "Not Found"; Duo = "Not Found";
        Updates = "Not Found"; UpdatesNeeded="Unresponsive to Ping";DNSServers="Not Found";EMET="Not Found";SCEP="Not Found";SCCM="Not Found";
        CarbonBlack="Not Found"; NxLog="Not Found"; Puppet="Not Found"; LAPS = "Not Found"; Sensu = "Not Found"; FWStatus = "Not Found";SMB1Enabled = "Not Found"} 
        }
    else
        {if ([bool](Test-WSMan $_))
            {$ServerInfo += New-Object psobject -Property @{PSComputerName = $_; OS = "Not Found"; CPUs = "Not Found"; Memory = "Not Found"; 
            WUServer = "Not Found"; AutoUpdate = "Not Found";LastBoot = "Not Found"; RebootPending = "Not Found"; LastPatch = "Not Found"; Duo = "Not Found";
            Updates = "Not Found"; UpdatesNeeded="Unresponsive to Invoke-Command";DNSServers="Not Found";EMET="Not Found";SCEP="Not Found";SCCM="Not Found";
            CarbonBlack="Not Found"; NxLog="Not Found"; Puppet="Not Found"; LAPS = "Not Found"; Sensu = "Not Found"; FWStatus = "Not Found";SMB1Enabled = "Not Found"}
            }
        Else
            {$ServerInfo += New-Object psobject -Property @{PSComputerName = $_; OS = "Not Found"; CPUs = "Not Found"; Memory = "Not Found"; 
            WUServer = "Not Found"; AutoUpdate = "Not Found";LastBoot = "Not Found"; RebootPending = "Not Found"; LastPatch = "Not Found"; Duo = "Not Found";
            Updates = "Not Found"; UpdatesNeeded="Unresponsive to Test-WSMan";DNSServers="Not Found";EMET="Not Found";SCEP="Not Found";SCCM="Not Found";
            CarbonBlack="Not Found"; NxLog="Not Found"; Puppet="Not Found"; LAPS = "Not Found"; Sensu = "Not Found"; FWStatus = "Not Found";SMB1Enabled = "Not Found"}
            }
        }
    $j++
...
$PatchStatus = $ServerInfo | select PSComputerName,OS,WUServer,AutoUpdate,LastBoot,RebootPending,LastPatch,Updates,UpdatesNeeded,EMET,SCEP,SCCM | sort PSComputerName
...
$Over90D = $Left2 | ? {(Get-Date $($_.LastPatch)) -lt (Get-Date).AddDays(-90)} 
$Left3 = @(); $Left2 | % {if ($Over90D -notcontains $_){$Left3+=$_}}
$Over30Updates = $Left3 | ?{$_.Updates -gt 30}
...
$PatchStatus| % `
    ...
    if ($_.OS -ne 'Not Found')
        ...
        if ($_.SCCM -eq $true) {$_.SCCM = "<mark><FONT COLOR=`"RED`">$($_.SCCM)</FONT></mark>"}
        if ($_.SCEP -eq $false) {$_.SCEP = "<mark><FONT COLOR=`"BurlyWood`">$($_.SCEP)</FONT></mark>"}
        if ($_.LastPatch) 
            {if ((Get-Date $($_.LastPatch)) -lt (Get-Date).AddDays(-90))
                {$_.LastPatch = "<mark><FONT COLOR=`"red`">$($_.LastPatch)</FONT></mark>"}
            }
        if ([int32]$_.Updates -gt 30) {$_.Updates = "<mark><FONT COLOR=`"Red`">$($_.Updates)</FONT></mark>"}
...
$ServerInfo = $ServerInfo | select DNSName,Name,OS,IPv4,DNSServers,CPUs,Memory,DeployedOn,OrganizationalUnit,PasswordLastSet,EMET,SCEP,SCCM,CarbonBlack,NxLog,Puppet,Duo,LAPS,Sensu,FWStatus, SMB1Enabled,WsusGroup,WUServer,AutoUpdate,LastBoot,RebootPending,LastPatch,Updates,UpdatesNeeded | Sort DNSName
...
$ServerInfo | % `
    ...
    if ($_.OS -ne 'Not Found')
        ...
        if ($_.EMET -eq $false) {$_.EMET = "<mark><FONT COLOR=`"BurlyWood`">$($_.EMET)</FONT></mark>"}
        if ($_.LAPS -eq $false) {$_.LAPS = "<mark><FONT COLOR=`"BurlyWood`">$($_.LAPS)</FONT></mark>"}
        if ($_.CarbonBlack -eq $false) {$_.CarbonBlack = "<mark><FONT COLOR=`"BurlyWood`">$($_.CarbonBlack)</FONT></mark>"}
        if ($_.NxLog -eq $false) {$_.NxLog = "<mark><FONT COLOR=`"BurlyWood`">$($_.NxLog)</FONT></mark>"}
        if ($_.Sensu -eq $false) {$_.Sensu = "<mark><FONT COLOR=`"BurlyWood`">$($_.Sensu)</FONT></mark>"}
        if ($_.Puppet -eq $false) {$_.Puppet = "<mark><FONT COLOR=`"BurlyWood`">$($_.Puppet)</FONT></mark>"}
        if ($_.SCCM -eq $true) {$_.SCCM = "<mark><FONT COLOR=`"RED`">$($_.SCCM)</FONT></mark>"}
        if ($_.SCEP -eq $false) {$_.SCEP = "<mark><FONT COLOR=`"BurlyWood`">$($_.SCEP)</FONT></mark>"}
        if (($_.Duo -eq $false) -and (($_.Name -match '-fintech') -or ($_.name -match '-rdsh'))) {$_.Duo = "Not Required"}
        if (($_.Duo -eq $false) -and (($_.Name -notmatch '-fintect') -and ($_.Name -notmatch '-RDSH'))) {$_.Duo = "<mark><FONT COLOR=`"BurlyWood`">$($_.Duo)</FONT></mark>"}

 	    if ($_.LastPatch)
            {if ((Get-Date $($_.LastPatch)) -lt (Get-Date).AddDays(-90))
                {$_.LastPatch = "<mark><FONT COLOR=`"Red`">$($_.LastPatch)</FONT></mark>"}}
	    if ([int]$_.Updates -gt 30)
            {$_.Updates = "<mark><FONT COLOR=`"Red`">$($_.Updates)</FONT></mark>"}
...
if ((Get-Date).DayOfWeek -eq 'Monday') #run installed applications only on Mondays
{
# get installed applications on DCs
$Begin = Get-Date
$ScriptB = {
Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* `
| ?{$_.DisplayName}|  Select-Object DisplayName, DisplayVersion, Publisher, InstallDate,InstallLocation,UninstallString
Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* `
| ?{$_.DisplayName}|  Select-Object DisplayName, DisplayVersion, Publisher, InstallDate,InstallLocation,UninstallString
}

$AllDCs = $DCs.DNSHostName
Invoke-Command -ComputerName $AllDCs -ScriptBlock $ScriptB -AsJob
Do {sleep 5
$leftServers = (Get-Job -IncludeChildJob | ?{$_.State -eq "running" -or $_.State -eq "NotStarted"}).count
write-progress -Activity 'Collect Installed applications Info' -percentComplete ((($DCs.Count - $leftServers)/$DCs.Count)*100) -Status "Completed $($DCs.Count - $leftServers) out of $($DCs.Count) Servers"
} 
Until (((Get-Job -IncludeChildJob | ?{$_.State -eq "running"}).count -eq 0) -or ((New-TimeSpan -Start $Begin -End (Get-Date)).TotalMinutes -gt 8))
$AllInstalledApps = Get-Job | Receive-Job
Get-Job | Stop-Job; Get-Job | Remove-Job
$AllInstalledApps = $AllInstalledApps | select PsComputerName,DisplayName,DisplayVersion,Publisher,InstallDate,UninstallString | Sort PsComputerName,DisplayName
$AllInstalledApps = $AllInstalledApps | ?{$_.displayname -notmatch "KB\d" -and $_.displayname -notmatch "language pack"}

$AllInstalledApps | Export-Csv $DataLocation3\ServerApps.csv -NoTypeInformation -Force

## build the detailed installed applications page in LCA1
$title = "applications Info DCs"
$TableID = "applicationsDCs"
$Caption = "Installed Applications Information DCs"

$htmlMiddle = ""
$htmlMiddle += "<div id='$title'>"
$ArrayToHtml = $AllInstalledApps | ConvertTo-HTML -fragment 
$ArrayToHtml = $ArrayToHtml -replace "&lt;","<" -replace "&quot;","`"" -replace "&gt;",">" -replace "&amp;#13","&#13" -replace "&amp;#10","&#10"
...
$HTMLmessage = (Build-Header -title $title) + $css + $HTMLMiddle + $HTMLEnd
	...
	$OutputLocation3 | % {$HTMLmessage | Out-File $_\AppsOnDCs$SaveFormatDate.html}
	
	# Now create the detail launch page
	$html = @()
	$html = "<html>`n"
	$html += "<head><title>Installed Apps on DCs</title>`n"
	$html += "<script type=""text/javascript"">`n"
	$html += "function open_win() `n"
 	$html += "{	var myEle=document.getElementById('gopage');`n"
	$html += "	var myiFrame=document.getElementById('iframe');`n"
	...
	$html += "</script></head>`n"
	$html += "<body>`n"
	$html += "<iframe id=""iframe"" width=""100%"" height=""95%"" src=""AppsOnDCs$SaveFormatDate.html""></iframe>`n"
	$html += "<br>Show Report from: &nbsp;<select id=""gopage"" onChange=""open_win()"">`n"
	$html += "	<option value="""" selected></option>`n"
	$Files = Get-ChildItem "$($OutputLocation3[0])\AppsOnDCs20*.html" | Sort CreationTime -Descending
	ForEach ($File in $Files)
	{	$FileName = $File.Name.Substring(13,2) + "/" + $File.Name.Substring(15,2) + "/" + $File.Name.Substring(9,4) + " " + $File.Name.Substring(17,2) + ":" + $File.Name.Substring(19,2)
		$html += "	<option value=""" + $File.Name + """>" + $FileName +"</option>`n"
	}
	$html += "</select></body></html>`n"
	$OutputLocation3 | % {$html | Out-File $_\AppsOnDCsIndex.html}
}