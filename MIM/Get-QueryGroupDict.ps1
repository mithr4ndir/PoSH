...
$Obj = $array | ? {$_.RuleType -notlike "*User Explicitly"} | ? {$_.RuleQuery -like "*memberof*"} | select -ExpandProperty RuleQuery
...
If ($InputFile) {
    foreach ($Key in $TheDict.Keys) { echo "$Key,$($TheDict[$Key])" >> $InputFile }
} Else {
